...
class TaskTest extends TestCase
{
    ...
    public function it_can_list_all_tasks_for_teacher_course()
    {
        ...
        $response = $this->withHeader('Authorization', 'Bearer ' . $token)
            ->getJson('/api/tasks?course_id=1'); // مثال: يمكن تعديل ID الدورة حسب الحاجة
...
// محاولة الحصول على ID المهمة التي تم إنشاؤها
    ...
    public function it_allows_teacher_to_store_note_for_assigned_task()
    {
        ...
        $response = $this->postJson("/api/notes/{$task->id}/users/{$student->id}/add-note?course_id={$course->id}", $data);
...
/** @test */
    public function it_allows_teacher_to_remove_note_for_assigned_task()
    {
        ...
        $response = $this->deleteJson("/api/notes/{$task->id}/users/{$student->id}/delete-note?course_id={$course->id}");
...
public function it_allows_forced_deletion_of_a_task_for_teacher()
{
    ...
    $response = $this->postJson("/api/tasks/{$task->id}/forcedelete?course_id={$course->id}");
